...
size HBox::measure(constraints c, Node&) const {
    uint32_t total_w = 0;
    uint32_t max_h   = 0;

    bool first = true;
    for (auto& child : children) {
        size cs = child.measure(c, const_cast<Node&>(child));
...
void HBox::layout(rect r, Node&) const {
    ...
    for (auto& child : children) {
        ...
        child.layout(child_rect, const_cast<Node&>(child));
        cursor_x += cs.width + spacing;
    }
}

void HBox::draw(draw::DrawCtx& ctx, Node&) const {
    for (auto& child : children) {
        child.draw(ctx, const_cast<Node&>(child));
    }
}
