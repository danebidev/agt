...
size HBox::measure(ui::constraints c, ui::Node&, ui::Data& data) {
    uint32_t total_w = 0;
    uint32_t max_h   = 0;

    bool first = true;
    for (auto& child : children) {
        size cs = child.measure(c, child, data);
...
void HBox::layout(rect r, ui::Node&, ui::Data& data) {
    ...
    for (auto& child : children) {
        ...
        child.layout(child_rect, child, data);
        cursor_x += cs.width + spacing;
    }
}

void HBox::draw(ui::Node&, ui::Data& data) {
    for (auto& child : children) {
        child.draw(child, data);
    }
}
